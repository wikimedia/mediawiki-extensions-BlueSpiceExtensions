...
Ext.onReady(function() {
	pmStore = new Ext.data.JsonStore({
		...
		url: bs.util.getAjaxDispatcherUrl( 'PermissionManager::getData' ),
		listeners: {
	...
	pmPivotGrid = new Ext.grid.PivotGrid({
		...
		buttons: [{
			text: mw.message( 'bs-permissionmanager-save' ).plain(),
			handler: function() {
				Ext.Ajax.request({
					url: bs.util.getAjaxDispatcherUrl( 'PermissionManager::setData' ),
					success: function(response, opts) {
						var obj = Ext.decode(response.responseText);
						if(obj.success) {
							pmPivotGrid.store.load();
							Ext.Msg.alert(mw.message( 'bs-permissionmanager-state' ).plain(), mw.message( 'bs-permissionmanager-allChangesSaved' ).plain());
						}
						else {
							Ext.Msg.alert(mw.message( 'bs-permissionmanager-error' ).plain(), obj.msg);
						}
					},
			...
			text: mw.message( 'bs-permissionmanager-cancel' ).plain(),
			handler: function() {
				Ext.Ajax.request({
					url: bs.util.getAjaxDispatcherUrl( 'PermissionManager::setDataAbort'),
					success: function(response, opts) {
						pmPivotGrid.store.load();
						Ext.Msg.alert(mw.message( 'bs-permissionmanager-state' ).plain(), mw.message( 'bs-permissionmanager-allChangesCanceled' ).plain());
					},
					failure: function(response, opts) {
						// CR MRG (01.07.11 13:01): i18n
						Ext.Msg.alert(mw.message( 'bs-permissionmanager-error' ).plain(), 'server-side failure with status code ' + response.status);
					}
				});
			}
		}],
		...
		tbar: [{
			text: mw.message( 'bs-permissionmanager-labelGroup' ).plain(),
			width: 80,
			id: 'pmViewChangerBtn',
			menu: {
				xtype: 'menu',
				...
				{
					xtype: 'menucheckitem',
					text: mw.message( 'bs-permissionmanager-labelNamespace' ).plain(),
					idxType: 'namespace',
					group: 'pm_vc'
				},
				{
					xtype: 'menucheckitem',
					text: mw.message( 'bs-permissionmanager-labelRight' ).plain(),
					idxType: 'permission',
					group: 'pm_vc'
				}
				],
			...
			store: new Ext.data.ArrayStore({
				autoLoad: true,
				fields: ['name', 'index'],
				url: bs.util.getAjaxDispatcherUrl( 'PermissionManager::getIndexData'),
				listeners: {
					beforeload: function(store, options) {
						options.params.idxType = pmPivotGrid.idxType;
					},
			...
			text: mw.message( 'bs-permissionmanager-labelTemplateEditor' ).plain(),
			handler: function() {
				...
				cellselect: {
					fn: function(model, rowIndex, columnIndex) {
						...
						if(pmPivotGrid.idxType == 'group') {
							...
							obj.index = mw.message( 'bs-permissionmanager-labelPermissions' ).plain();
						...
						if(cell.edited == cell.checked) {
							...
							Ext.Ajax.request({
								url: bs.util.getAjaxDispatcherUrl( 'PermissionManager::setDataTemporary'),
								success: function(response, opts) {
		...
		showTemplateEditor: function() {
			if(!pmTemplateEditor) {
				pmTemplateEditor = new Ext.Window({
					...
					{
						xtype: 'button',
						text: mw.message( 'bs-permissionmanager-labelAdd' ).plain(),
						id: 'pmTemplateEditorAddButton',
						handler: function() {
							pmTemplateEditor.processNode(pmTemplateEditor.workingTemplate);
							Ext.Msg.prompt(mw.message( 'bs-permissionmanager-labelAdd' ).plain(), mw.message( 'bs-permissionmanager-labelTitle' ).plain(), function(btn, text){
								if (btn == 'ok'){
									if (text == '') {
										Ext.Msg.alert(mw.message( 'bs-permissionmanager-error' ).plain(), mw.message( 'bs-permissionmanager-templateNameIsEmpty' ).plain());
										return;
									}
									if (Ext.getCmp('pmTemplateEditorTree').getRootNode().findChild('text', text)) {
										Ext.Msg.alert(mw.message( 'bs-permissionmanager-error' ).plain(), mw.message( 'bs-permissionmanager-templateNameExists' ).plain());
										return;
									}
					...
					{
						xtype: 'button',
						text: mw.message( 'bs-permissionmanager-labelEdit' ).plain(),
						disabled: true,
						id: 'pmTemplateEditorEditButton',
						handler: function() {
							pmTemplateEditor.processNode(pmTemplateEditor.workingTemplate);
							if(pmTemplateEditor.workingTemplate) {
								Ext.Msg.prompt(mw.message( 'bs-permissionmanager-labelEdit' ).plain(), mw.message( 'bs-permissionmanager-labelTitle' ).plain(), function(btn, text){
									if (btn == 'ok'){
										if (text == '') {
											Ext.Msg.alert(mw.message( 'bs-permissionmanager-error' ).plain(), mw.message( 'bs-permissionmanager-templateNameIsEmpty' ).plain());
											return;
					...
					{
						xtype: 'button',
						text: mw.message( 'bs-permissionmanager-labelRemove' ).plain(),
						disabled: true,
					...
					items: [
					{
						...
						useArrows: true,
						margins: '5 0 5 5',
						dataUrl: bs.util.getAjaxDispatcherUrl( 'PermissionManager::getTemplateData'),
						root: {
							text: 'Tree Node'
						},
						listeners: {
					...
					},
					{
						...
						items: [
						{
							...
							items: [
							{
								...
								fieldLabel: mw.message( 'bs-permissionmanager-labelDescription' ).plain(),
								anchor: '100%',
								height: 71,
								listeners: {
									focus: function() {
						...
						},
						{
							...
							items: [{
								xtype: 'dataview',
								store: new Ext.data.ArrayStore({
									url: bs.util.getAjaxDispatcherUrl( 'PermissionManager::getPermissionArray'),
					...
					saveNodes: function() {
						...
						Ext.Ajax.request({
							...
							success: function(response, opts) {
								var obj = Ext.decode(response.responseText);
								pmPivotGrid.store.load();
								Ext.Msg.alert(mw.message( 'bs-permissionmanager-state' ).plain(), obj.msg);
