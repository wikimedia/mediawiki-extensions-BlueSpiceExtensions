 ...
 * @version    $Id: ReviewProcess.class.php 9103 2013-04-08 11:53:18Z pwirth $
...
//	var $editable;
	...
	static function newFromRequest() {
		...
		$oReviewProcess->pid = BsCore::getParam('rv_pid', 0, BsPARAM::REQUEST | BsPARAMTYPE::NUMERIC);

		// starttag
		//$hw_review->mode = $_REQUEST['rv_mode'];
		// rv_mode is stored in table hw_review as a varchar(20)
		$oReviewProcess->mode = BsCore::getParam('rv_mode', '', BsPARAM::REQUEST | BsPARAMTYPE::STRING);
		/*
		...
		$oReviewProcess->startdate = date("Y-m-d", strtotime(BsCore::getParam('rv_startdate', '', BsPARAM::REQUEST | BsPARAMTYPE::STRING)));
		//$hw_review->enddate = date("Y-m-d", strtotime($_REQUEST['rv_enddate']));
		$oReviewProcess->enddate = date("Y-m-d", strtotime(BsCore::getParam('rv_enddate', '', BsPARAM::REQUEST | BsPARAMTYPE::STRING)));
		...
		//}
		$paramRvStepName = BsCore::getParam('rv_step_name', array(), BsPARAM::REQUEST | BsPARAMTYPE::ARRAY_STRING);

		$countParamRvStepName = count($paramRvStepName);
		if ($countParamRvStepName > 0) { // count($var) == 0 <==> Variable not set OR Array is empty
			$paramRvStepComment = BsCore::getParam('rv_step_comment', array(), BsPARAM::REQUEST | BsPARAMTYPE::ARRAY_STRING);
			$paramRvStepStatus = BsCore::getParam('rv_step_status', array(), BsPARAM::REQUEST | BsPARAMTYPE::ARRAY_NUMERIC | BsPARAMOPTION::DEFAULT_ON_ERROR);
		}
	...
	static function newFromJSON($sJSON = '', &$aErrors) {
		...
		foreach ($paramRvSteps as $oStep) {
			...
			$oReviewProcess->steps[] = BsReviewProcessStep::newFromData(
					BsCore::sanitize($oStep->userid, '', BsPARAMTYPE::STRING),
					$sComment,
	...
	static function listReviews($uid, $bIgnoreStatus = true) {
		...
		$aOptions = array('GROUP BY' => join(',', $aRevColumns) . ',' . join(',', $aRevsColumns));
		$sFields = join(',', $aRevColumns) . ',' . join(',', $aRevsColumns);
