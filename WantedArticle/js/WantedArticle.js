 ...
 * @version    $Id: WantedArticle.js 9415 2013-05-16 13:01:38Z rvogel $
...
BsWantedArticle = {
	...
	checkArticleTitle: function( sArticleTitle, sDefault ) { // TODO RBV (06.10.10 09:42): Should be in common framework
		if ( sArticleTitle == '' || sArticleTitle == sDefault ) {
			BlueSpice.alert( mw.msg('bs-wantedarticle-info_nothing_entered') )
			return false;
		}
		...
		if( aFoundChars.length > 0 ) {
			BlueSpice.alert( mw.msg('bs-wantedarticle-info_title_contains_invalid_characters' ) + aFoundChars.join( ', ' ) );
			return false;
		}
		return true;
	},
	...
	sendSuggestion: function( sArticleTitle ) {
		...
		$.getJSON( 
			...
			{'suggestion' : sArticleTitle},
			function( oData, oTextStatus ) {
				BlueSpice.alert( oData.message );
	...
	reloadAllWantedArticleTags: function() {
		$( '.bs-wantedarticle-tag' ).each( function() {
			...
			$.getJSON( 
				BlueSpice.buildRemoteString( 'WantedArticle', 'ajaxGetWantedArticles' ),
				{ 
